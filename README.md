...
## Projects 📱
- **Maliyar** [`Website`](https://maliyar.thepixelforge.app) | [`TestFlight (iOS)`](https://testflight.apple.com/join/3Kq77BAq) | [`Cafe Bazaar (Android)`](https://cafebazaar.ir/app/app.thepixelforge.loan_tracker): *An **offline-first** Persian/Farsi loan and installment manager. Helps users track loans, schedule installments, and monitor upcoming payments — all on-device, with no account required. Full **RTL layout**, **Persian (Jalali) calendar** support, and secure local storage. Built cross-platform with **Flutter** / **Dart** using **Clean Architecture** + feature-first folders, **Riverpod** for state management, **Drift + SQLite** for local persistence, **GoRouter** (`StatefulShellRoute`) for navigation, `fl_chart` for analytics, **`flutter_local_notifications`** + `timezone` for installment reminders, **`flutter_secure_storage`** + `local_auth` + `crypto` for PIN / biometric app-lock. Distributed via **App Store**, **TestFlight**, and **Cafe Bazaar**.*
